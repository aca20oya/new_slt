...
from signjoey.data import load_data, make_data_iter, load_dgs, load_mono_dgs
...
def validate_on_data(
    ...
    with torch.no_grad():
        ...
        if do_recognition:
            ...
            if dataset_version == "phoenix_2014_trans" or dataset_version == "dgs_corpus":
...
def test(
    ...
    # load the data
    if dataset_version == "dgs_corpus":
        _, dev_data, test_data, gls_vocab, txt_vocab = load_dgs(data_cfg=cfg["data"])
    else:
...
def translate(cfg_file: str, ckpt: str, output_path: str = None, openpose_path: str = None,
    ...
    use_cuda = cfg["training"].get("use_cuda", False)
    level = cfg["data"]["level"]
    dataset_version = cfg["data"].get("version", "dgs_corpus")
    translation_max_output_length = cfg["training"].get(
        "translation_max_output_length", None
    )

    # The Mono dataset bit, this is for when I create my own videos
    #if not sys.stdin.isatty():
        # Create Mono dataset, input is path to features.
        #test_data = load_mono(sys.stdin)
    
    # Load vocab and singular dataset
    translate_data, gls_vocab, txt_vocab = load_mono_dgs(data_cfg=cfg["data"], path=openpose_path)
    ...
    if do_recognition:
        ...
        logger.info("finished in %.4fs ", time.time() - valid_start_time)
        reference = results["gls_ref"]
        hypotheses = results["gls_hyp"]
    ...
    if do_translation:
        ...
        reference = results["txt_ref"]
        hypotheses = results["txt_hyp"]
