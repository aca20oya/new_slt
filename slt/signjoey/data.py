...
def load_data(data_cfg: dict) -> (Dataset, Dataset, Dataset, Vocabulary, Vocabulary):
    ...
    def tokenize_features(features):
        # print("Before preprocessing shape: ", np.array(features).shape)
        # print("Before preprocessing array: ", features)
        ft_list = torch.split(features, 1, dim=0)
        # print("After preprocessing shape: ", np.array([ft.squeeze() for ft in ft_list]).shape)
        # print("After preprocessing array: ", [ft.squeeze() for ft in ft_list])
        return [ft.squeeze() for ft in ft_list]

    # NOTE (Cihan): The something was necessary to match the function signature.
    def stack_features(features, something):
        # print("Before postprocessing: ", np.array(features).shape)
        # print("After postprocessing: ", np.array(torch.stack([torch.stack(ft, dim=0) for ft in features], dim=0)).shape)
...
# Gets a random sample from the test dataset, We'll use this to test the translate feature
def load_one(data_cfg: dict) -> (Dataset):
    ...
    remove_ratio = 1 / len(test_data)
    keep, _ = test_data.split(
            split_ratio=[remove_ratio, 1 - remove_ratio], random_state=random.getstate()
        )
    one_example = keep

    # For printing the contents of the dataset (which contains just one sample). This prints the text.
    for thing in one_example:
        print ("THIS IS KEEP: ", thing.txt)
        print ("THIS IS GLOSS: ", thing.gls)
        print ("TIS IS SIGN: ", np.array(thing.sgn).shape)

    gls_field.vocab = gls_vocab
    txt_field.vocab = txt_vocab
    return one_example, gls_vocab, txt_vocab
